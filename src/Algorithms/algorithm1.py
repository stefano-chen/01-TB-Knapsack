...
def iterative_algorithm_p(n_items, capacity, weights, profits, survive_probs, mt):
    ...
    model = ModelP(j=0, n=n_items, c=capacity, w=weights, p=profits, q=survive_probs,
    ...
    while best_heu_val < last_exact_profit and (time.time() - start_time) < mt:
        ...
        if best_heu_val < last_exact_profit:
            val = calculate_expected_profit(sol, profits, survive_probs)
            if val > best_heu_val:
                best_heu_sol = sol
                best_heu_val = val
            previous_solutions.append(sol)
            j+=1
            model = ModelP(j=j, n=n_items, c=capacity, w=weights, p=profits, q=survive_probs,
                           best_heu_value=best_heu_val, last_exact_profit=last_exact_profit,
                           previous_solutions=previous_solutions, ml=1)
